...
test('ダークモードのトグルボタンが表示され、クリックで切り替わる', async ({ page }) => {
  await page.goto('/');

  const toggleButton = page.getByRole('button', { name: 'ダークモードに切り替え' });
  await expect(toggleButton).toBeVisible();

  await test.step('ダークモードに切り替えると背景色が変わる', async () => {
  ...
  await test.step('ライトモードに戻すと背景色が元に戻る', async () => {
