...
test('タスクを削除できる', async ({ page }) => {
  ...
  // UIでタスクを作成
  await page.getByTestId('add-task-button').click();
  await page.getByTestId('task-name-input').fill('削除テスト用タスク');
  await page.getByTestId('save-button').click();
...
test('削除の確認でキャンセルするとタスクが残る', async ({ page }) => {
  ...
  // UIでタスクを作成
  await page.getByTestId('add-task-button').click();
  await page.getByTestId('task-name-input').fill('削除キャンセルテスト');
  await page.getByTestId('save-button').click();
...
test('添付ファイル付きタスクを削除すると添付URLにアクセスできなくなる', async ({ page }) => {
  ...
  // UIでタスクを作成
  await page.getByTestId('add-task-button').click();
  await page.getByTestId('task-name-input').fill('添付削除テスト');
  ...
  await expect(page.getByTestId('attachments-list').getByText('delete-attach.png')).toBeVisible();
  ...
  // 備考欄から添付URLを抽出
  const notesValue = await page.getByTestId('notes-input').inputValue();
