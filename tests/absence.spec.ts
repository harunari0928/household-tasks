...
/**
 * タスクをフォームから作り、「今すぐカンバンに起票」でカードをカンバンに置く。
...
async function createTaskWithAbsenceBehavior(
  ...
  await page.getByText(options.name).click();
  await page.getByRole('button', { name: '今すぐカンバンに起票' }).click();
  await page.getByText('カンバンボードに追加しました').waitFor();
...
test.describe('不在日のカンバン表示', () => {
  test('不在日は「不在中は非表示」のタスクをカンバンから消す', async ({ page, baseURL }) => {
    // Arrange
    await createTaskWithAbsenceBehavior(page, {
  ...
  test('不在日でも「不在でも表示」のタスクはカンバンに残す', async ({ page, baseURL }) => {
    // Arrange
    await createTaskWithAbsenceBehavior(page, {
  ...
  test('不在日ではない日は「不在中は非表示」のタスクもカンバンに表示する', async ({ page, baseURL }) => {
    // Arrange
    await createTaskWithAbsenceBehavior(page, {
      name: '浴槽掃除', category: 'water', behaviorLabel: '不在中は非表示',
    });
  ...
  test('不在日は予定名つきのお知らせを表示する', async ({ page, baseURL }) => {
    // Arrange
  ...
  test('不在日ではない日はお知らせを表示しない', async ({ page, baseURL }) => {
    // Arrange
    await setAbsenceDays(page, baseURL!, [{ date: addDays(getTodayJST(), 3) }]);
...
test.describe('家族カレンダーからの不在日の同期', () => {
  ...
  test('次の同期に含まれない不在日は消える', async ({ page, baseURL }) => {
    // Arrange
    const today = getTodayJST();
    await setAbsenceDays(page, baseURL!, [{ date: today, summary: '沼津旅行' }]);
    await goToKanban(page);
