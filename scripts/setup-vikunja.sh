...
ENV_FILE="${2:-.env}"
USERNAME="${VIKUNJA_ADMIN_USERNAME:-admin}"
EMAIL="${VIKUNJA_ADMIN_EMAIL:-admin@localhost.local}"
PASSWORD="${VIKUNJA_ADMIN_PASSWORD:-changeme123}"
...
TOKEN_RESPONSE=$(curl -s -X PUT "${VIKUNJA_URL}/api/v1/tokens" \
  ...
  -H "Content-Type: application/json" \
  -d "{\"title\":\"scheduler\",\"permissions\":{\"projects\":[\"read_all\"],\"tasks\":[\"create\",\"read_all\",\"update\"]},\"expires_at\":\"${EXPIRES_AT}\"}")
